...
void* producer(void* args){
    while(1){ // without while loop program works fine
        ...
        // Add to the buffer
        sem_wait(&semEmpty); // so that semaphore start decrementing from the 10
        pthread_mutex_lock(&mutexBuffer);   // Mutex added to avoid deadlock 
        // as we can see here that the if statement is removed from here bcs the semaphore sem_wait will decrement from 10. once it reaches 0 then it will wait until the value incremented again by sem_post
        // so semaphore itseld taking care of buffer full and empty issues so no need of the conditional statements here
        buffer[count] = x;
        count++;
        pthread_mutex_unlock(&mutexBuffer);
        sem_post(&semFull);// so that semaphore start incrementing from 0
    }
    //return NULL;
}
...
void* consumer(void* args){
    while(1){ // without while loop program works fine
        //int y = -1; this is alo not needed now bcs of semaphore
        int y;
        // Remove from the buffer
        sem_wait(&semFull);
        pthread_mutex_lock(&mutexBuffer);
        // as we can see here that the if statement is removed from here bcs the semaphore sem_wait will semfull. Which was incremented in the producer routine
...
int main(int argc, char* argv[]){
    ...
    sem_init(&semEmpty, 0, 10); // bcs of we have 10 elements in buffer
    sem_init(&semFull, 0, 0);   // bcs semFull should be so that we can stack 10 elements
    int i;
